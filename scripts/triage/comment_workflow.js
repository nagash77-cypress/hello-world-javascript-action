...
async function handleComment(github, context) {
  ...
  if (!context.payload.comment || context.payload.comment.user.type === 'Bot') {
    console.log(
      'Payload does not contain a comment event made by a human user.'
    )
    return
  }
  ...
  const issueOrPullRequest = pull_request
  ...
  if (isCommentFromMember) {
    console.log(
      `Comment came from a member of the Org.  isCommentFromMember var: ${isCommentFromMember}`
    )
  ...
  if (!isCommentFromMember) {
    ...
    const issueDataFromGraphQL =
      getItemInfo.organization.repository[issueOrPullRequest.type]
    ...
    let projectItemID =
      issueDataFromGraphQL.projectItems.nodes.length > 0
        ? issueDataFromGraphQL.projectItems.nodes[0].id
        : null
    const isItemArchived =
      issueDataFromGraphQL.projectItems.nodes.length > 0
        ? issueDataFromGraphQL.projectItems.nodes[0].isArchived
        : false
    const statusFieldID = getItemInfo.organization.projectV2.field.id
    const newStatusColumnID = getItemInfo.organization.projectV2.field.options.find(
      (option) => option.name === 'New Issue'
    ).id
    ...
    if (isItemArchived) {
      ...
      const unarchiveItem = await github.graphql(
        unarchiveQuery,
        unarchiveQueryVars
      )
    }
    ...
    if (issueDataFromGraphQL.closed) {
      ...
      const commentOnClosedItem = await github.graphql(
        commentOnClosedItemQuery,
        commentOnClosedItemQueryVars
      )
    }
  }
}
